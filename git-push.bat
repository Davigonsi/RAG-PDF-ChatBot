...
set /p repo_name="Enter GitHub repository (format: Davigonsi/Watermark-E-signatureApp): "

REM Validate input format
echo %repo_name% | findstr /r "^[^/]*/[^/]*$" >nul
if errorlevel 1 (
    echo ERROR: Invalid format. Please use format: Davigonsi/Watermark-E-signatureApp
...
if "%commit_msg%"=="" (
  git commit -m "Initial commit - Photo Filter Booth with webcam, filters, and comparison view"
) else (
